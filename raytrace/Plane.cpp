...
bool Plane::intersect(const Ray& r, HitInfo& hit, unsigned int prim_idx) const
{
  ...
  //       the dot product of the vectors v and w.
	...
	auto q = dot(r.direction, get_normal());
	if (fabsf(q) < 1.0e-12f) return false;

	float t = -(dot(r.origin, get_normal())+d)/q;
	if (t > r.tmax || t < r.tmin) return false;
...
{ 
  ...
  const float3 originOffset = hit_pos - position;
  u = dot(onb.m_tangent, originOffset) / tex_scale;
  v = dot(onb.m_binormal, originOffset) / tex_scale;
