...
bool TriMesh::intersect(const Ray& r, HitInfo& hit, unsigned int prim_idx) const{
    ...
    const float3 p1 = geometry.vertex(face.x);
    const float3 p2 = geometry.vertex(face.y);
    const float3 p3 = geometry.vertex(face.z);

    float t, v, w;
    float3 hitNormal = make_float3(0);

    const bool hasHit = ::intersect_triangle(r, p1, p2, p3, hitNormal, t, v, w);
    ...
    hit.geometric_normal = normalize(-hitNormal);
    hit.material = &materials[mat_idx[prim_idx]];

    if (has_normals()) {
        hit.shading_normal = normalize(
            (1 - v - w) * normals.vertex(face.x) +
            v * normals.vertex(face.y) +
            w * normals.vertex(face.z)
        );
    }
    else {
